...
export default function App() {
  ...
  const [matchWomenOnly, setMatchWomenOnly] = useState(false); // AJOUTÉ POUR LE MATCH "UNIQUEMENT FEMME"
  ...
  const matchedBuddiesList = registeredUsers.filter((u) => {
    if (u.id === user?.id) return false;
    if (matchWomenOnly && u.gender === 'M') return false; // Applique le filtre "uniquement femme" dans le match
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col font-sans select-none">
      ...
      {/* POP-UP MATCHMAKING PARTNER (AVEC FILTRE UNIQUEMENT FEMME) */}
      {isMatchModalOpen && (
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-3xl max-w-md w-full p-5 space-y-4 shadow-2xl">
            ...
            <div className="space-y-2 pt-2 border-t border-neutral-800 max-h-60 overflow-y-auto">
              ...
              {matchedBuddiesList.length === 0 ? (
                <div className="text-center py-6 text-neutral-500 text-xs">Aucun athlète ne correspond à ces critères.</div>
              ) : (
