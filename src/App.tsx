...
// Imports depuis tes nouveaux fichiers !
import { 
...
export default function App() {
  ...
  // ==========================================
  // TOUTES LES FONCTIONS UTILITAIRES ET MÉTIER 
  // ==========================================
  ...
  const handleNextStory = () => { if (activeStoryIndex === null) return; if (activeStoryIndex < friendStoriesList.length - 1) { setActiveStoryIndex(activeStoryIndex + 1); setStoryProgress(0); setStoryCommentInput(''); } else { setActiveStoryIndex(null); } };
  const handlePrevStory = () => { if (activeStoryIndex === null) return; if (activeStoryIndex > 0) { setActiveStoryIndex(activeStoryIndex - 1); setStoryProgress(0); setStoryCommentInput(''); } else { setStoryProgress(0); } };
  const stopCameraStream = () => { if (streamRef.current) { streamRef.current.getTracks().forEach((t) => t.stop()); streamRef.current = null; } setIsCameraActive(false); if (cameraTarget === 'story') { setIsCreatingStory(true); } };
  const startCameraHandler = (target: any) => { if (target === 'story') { setIsCreatingStory(false); } setCameraTarget(target); setIsCameraActive(true); };
  
  const handleDeletePost = async (postId: string) => { if (!window.confirm("Supprimer cette publication ?")) return; const { error } = await supabase.from('posts').delete().eq('id', postId); if (!error) { setPosts((prev) => prev.filter((p) => p.id !== postId)); alert("Publication supprimée."); } };
  ...
  const handleSelectBuddyChat = (friend: RealUser) => {
    ...
    if (user) { const now = Date.now(); const updated = { ...lastReadTimestamps, [friend.id]: now }; setLastReadTimestamps(updated); try { localStorage.setItem('fitpulse_last_read_map', JSON.stringify(updated)); } catch(e) {} }
  ...
  const handleForgotPassword = async (e: React.FormEvent) => { e.preventDefault(); if (!email) return; setAuthLoading(true); const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: window.location.origin }); setAuthLoading(false); if (error) alert("Erreur : " + error.message); else setForgotPasswordSent(true); };
  const handleUpdatePassword = async (e: React.FormEvent) => { e.preventDefault(); if (!password || password !== confirmPassword) return; setAuthLoading(true); const { error } = await supabase.auth.updateUser({ password }); setAuthLoading(false); if (error) alert("Erreur : " + error.message); else { alert("Mot de passe mis à jour !"); setIsResetPasswordMode(false); setPassword(''); setConfirmPassword(''); window.history.replaceState({}, document.title, window.location.pathname); } };
  ...
  const switchCameraFacing = async () => {
    ...
    try { if (streamRef.current) streamRef.current.getTracks().forEach((t) => t.stop()); const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: newFacing }, audio: false }); streamRef.current = stream; if (videoRef.current) videoRef.current.srcObject = stream; } catch (err) {}
  };

  const capturePhoto = () => {
    if (!videoRef.current) return;
    const video = videoRef.current; const canvas = document.createElement('canvas'); canvas.width = video.videoWidth || 640; canvas.height = video.videoHeight || 480; const ctx = canvas.getContext('2d'); if (!ctx) return;
    ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
    canvas.toBlob((blob) => {
      if (!blob) return; const previewUrl = URL.createObjectURL(blob);
  ...
  const handleAddPR = (e: React.FormEvent) => { e.preventDefault(); if (!newPrExercise.trim() || newPrWeight === '' || newPrReps === '') return; setPersonalRecords([{ exercise: newPrExercise.trim(), weight: Number(newPrWeight), reps: Number(newPrReps), date: new Date().toISOString().split('T')[0] }, ...personalRecords]); setNewPrExercise(''); setNewPrWeight(''); setNewPrReps(''); alert('🏆 Nouveau record enregistré avec succès !'); };
  const handleSaveWeeklyPlanEdit = (index: number) => { const updated = [...weeklyPlan]; updated[index] = { ...updated[index], focus: editFocus, exercisesText: editExercisesText }; setWeeklyPlan(updated); setEditingDayIndex(null); };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (typingChannelRef.current && user && selectedBuddyChat) {
      ...
      typingTimeoutRef.current = setTimeout(() => { typingChannelRef.current.send({ type: 'broadcast', event: 'typing', payload: { userId: user.id, isTyping: false } }); }, 2000);
    }
  };
