...
// Fonction utilitaire pour calculer le statut de présence en fonction de la dernière activité
...
export default function ChatTab({
  ...
  // Auto-scroll vers le bas lors de l'envoi/réception de messages
  useEffect(() => {
    messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' });
  }, [currentChatMessages, isOtherUserTyping]);

  // Si aucun chat n'est ouvert, afficher la liste des amis
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-5 space-y-4 shadow-xl">
          ...
          <div className="space-y-3 pt-2">
            {activeChatUsers.length === 0 ? (
              ...
              activeChatUsers.map((buddy) => {
                const status = getUserStatus(buddy.updated_at || buddy.last_seen);
                return (
                  <div 
                    key={buddy.id} 
                  ...
                  >
                    <div className="flex items-center gap-3.5 relative">
                      <div className="relative">
                        ...
                        {/* Indicateur de statut (Vert, Rouge ou Gris) */}
  ...
  const buddyStatus = getUserStatus(selectedBuddyChat.updated_at || selectedBuddyChat.last_seen);

  // Dernier message envoyé par l'utilisateur connecté pour afficher le "Vu"
          ...
          currentChatMessages.map((msg, index) => {
            ...
            return (
              <div key={msg.id || index} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                ...
                {/* Indicateur "Vu" style Messenger affiché sous le tout dernier message ouvert */}
                {isLastMyMsg && (
