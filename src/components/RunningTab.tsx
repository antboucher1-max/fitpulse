...
const runnerIcon = L.divIcon({
  ...
  html: `<div style="width: 22px; height: 22px; background: #10b981; border: 4px solid #ffffff; border-radius: 50%; box-shadow: 0 0 16px #10b981;"></div>`,
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl max-w-md w-full p-6 space-y-5 shadow-2xl relative">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-neutral-400 leading-relaxed">
            Interface scindée : un Cockpit Live ultra-épuré pour courir, et un QG d'analyse post-effort blindé.
          </p>
        </div>
        <button
          type="button"
...
export default function RunningTab({
  ...
  const [activeTabSection, setActiveTabSection] = useState<'none' | 'circuits' | 'terrain' | 'ravito' | 'gear'>('none');

  // États de course Live
  ...
  const handleGenerateSmartCircuit = async (targetKm: number) => {
    ...
    try {
      ...
      const res = await fetch(`https://router.project-osrm.org/route/v1/foot/${baseLng},${baseLat};${baseLng + lngOffset},${baseLat + latOffset};${baseLng},${baseLat}?overview=full&geometries=geojson`);
  ...
  const handleFetchAllForestPaths = async () => {
    ...
    try {
      const res = await fetch(`https://router.project-osrm.org/route/v1/foot/3.325,50.505;3.34,50.515;3.325,50.505?overview=full&geometries=geojson`);
      const data = await res.json();
      if (data?.routes?.[0]) {
        setPlannedRoutePositions(data.routes[0].geometry.coordinates.map((c: [number, number]) => [c[1], c[0]]));
        setPlannedDistanceKm(Number((data.routes[0].distance / 1000).toFixed(2)));
        alert("✅ Parcours Trail généré !");
      }
    } catch {
  ...
  // Parseur GPX avec filtre altimétrique avancé (Deadband 2.5m)
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (tracks?.points?.length > 0) {
          ...
          alert(`GPX importé : ${km} km, +${Math.round(dPlus)}m D+ certifiés ⛰️`);
        }
      } catch {
        ...
        watchId = navigator.geolocation.watchPosition(
          (pos) => {
            ...
            if (delta > 0.002) {
              ...
              setActualDPlus(d => d + Math.round(delta * 15));
            }
          },
          () => {},
  ...
  return (
    <div className="space-y-6 pb-24 animate-fadeIn">
      ...
      {/* =========================================================================
          VUE A : COCKPIT LIVE (S'affiche uniquement quand on court)
          ========================================================================= */}
      {isRunning ? (
        <div className="bg-neutral-950 border border-emerald-500/50 rounded-3xl p-6 space-y-5 shadow-2xl animate-fadeIn">
          <div className="flex justify-between items-center">
            <span className="text-xs font-black uppercase text-emerald-400 flex items-center gap-2 animate-pulse">
              <Activity className="w-4 h-4" /> Cockpit Live (Mode Focus Actif)
            </span>
          ...
          <div className="w-full h-80 rounded-2xl overflow-hidden border border-neutral-800">
          ...
          <div className="grid grid-cols-4 gap-2 text-center">
            <div className="bg-neutral-900 p-3 rounded-2xl border border-neutral-800">
              <span className="text-[9px] text-neutral-400 block font-bold uppercase">Distance</span>
              <span className="text-base font-black text-white">{distanceKm.toFixed(2)} km</span>
            </div>
            <div className="bg-neutral-900 p-3 rounded-2xl border border-neutral-800">
              <span className="text-[9px] text-neutral-400 block font-bold uppercase">D+</span>
              <span className="text-base font-black text-emerald-400">+{actualDPlus}m</span>
            </div>
            <div className="bg-neutral-900 p-3 rounded-2xl border border-neutral-800">
              <span className="text-[9px] text-neutral-400 block font-bold uppercase">Vitesse</span>
              <span className="text-base font-black text-cyan-400">{currentSpeed}</span>
            </div>
            <div className="bg-neutral-900 p-3 rounded-2xl border border-neutral-800">
              <span className="text-[9px] text-neutral-400 block font-bold uppercase">Allure</span>
              <span className="text-base font-black text-orange-400">{paceFormatted}</span>
            </div>
          </div>

          <div className="flex gap-3 pt-2">
            <button onClick={() => setIsPaused(!isPaused)} className="flex-1 py-4 bg-neutral-800 hover:bg-neutral-700 text-white font-bold rounded-2xl text-xs cursor-pointer">
              {isPaused ? 'Reprendre 🏃' : 'Pause ⏸️'}
            </button>
            <button onClick={() => { setIsRunning(false); setIsReportModalOpen(true); }} className="flex-1 py-4 bg-red-950 border border-red-900 hover:bg-red-900 text-red-400 font-black rounded-2xl text-xs uppercase cursor-pointer">
              Terminer 🏁
            </button>
          </div>
        </div>
        ...
        /* =========================================================================
           VUE B : QG D'ANALYSE & LAB (S'affiche au repos / post-effort)
           ========================================================================= */
        <div className="space-y-6 animate-fadeIn">
          {/* Bloc d'Action de Démarrage */}
          <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6 space-y-4 shadow-2xl">
            <div className="flex justify-between items-center">
              <div>
                <span className="text-[10px] font-black uppercase tracking-widest text-emerald-400">QG Running & Lab</span>
                <h2 className="text-xl font-black text-white tracking-tight pt-0.5">Centre de Commande</h2>
              </div>
            ...
            <button onClick={() => { setIsRunning(true); setIsPaused(false); setSeconds(0); setDistanceKm(0); setActualDPlus(0); }} className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-2xl text-xs uppercase tracking-wider shadow-xl transition cursor-pointer flex items-center justify-center gap-2">
              <Play className="w-4 h-4 fill-white" /> Démarrer la session Live (Mode Focus) 🚀
            </button>
          </div>

          <FitBotSNC readinessScore={78} weeklyLoad={45} />

          {/* Tiroirs du QG (Analyses, Circuits, Ravitaillement, Gear Tracker) */}
          <div className="space-y-3">
            {/* 1. Architecte de Circuits */}
            <div className="bg-neutral-900 border border-neutral-800 rounded-3xl overflow-hidden shadow-xl">
              <button onClick={() => setActiveTabSection(activeTabSection === 'circuits' ? 'none' : 'circuits')} className="w-full p-4 flex justify-between items-center text-xs font-bold text-sky-400 cursor-pointer">
                <span className="flex items-center gap-2"><CompassIcon className="w-4 h-4" /> Architecte de Circuits & Itinéraires ⚡</span>
                {activeTabSection === 'circuits' ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
              </button>
              {activeTabSection === 'circuits' && (
                <div className="p-4 pt-0 border-t border-neutral-800 space-y-3 text-xs">
                  ...
                  {plannedRoutePositions.length > 0 && (
                    <button onClick={() => { onSaveRunPost?.(`🗺️ [PARCOURS DÉFI] Boucle de ${plannedDistanceKm} km. Qui vient ? 🚀`, plannedDistanceKm); alert("Partagé !"); }} className="w-full py-2.5 bg-sky-600 hover:bg-sky-500 text-white font-bold rounded-xl cursor-pointer">
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-3xl overflow-hidden shadow-xl">
              <button onClick={() => setActiveTabSection(activeTabSection === 'terrain' ? 'none' : 'terrain')} className="w-full p-4 flex justify-between items-center text-xs font-bold text-cyan-400 cursor-pointer">
                <span className="flex items-center gap-2"><Mountain className="w-4 h-4" /> Import GPX (Filtre Alti Avancé) 🗺️</span>
                {activeTabSection === 'terrain' ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
              </button>
              {activeTabSection === 'terrain' && (
                <div className="p-4 pt-0 border-t border-neutral-800 space-y-2 text-xs">
                  <label className="text-neutral-400 font-bold block">Importer votre fichier .GPX de montre :</label>
