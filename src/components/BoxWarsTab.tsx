...
import { Zap, Flame, Trophy, Plus, X, Dumbbell, Timer, MessageSquareText, Award, ShieldAlert } from 'lucide-react';
...
export default function BoxWarsTab({ currentUserId, currentUsername }: BoxWarsTabProps) {
  const [boxSubTab, setBoxSubTab] = useState<'wods' | 'prs' | 'feed'>('wods');
  ...
  // Box feed announcements
  const [boxAnnouncements, setBoxAnnouncements] = useState<any[]>([
    { id: 1, title: 'Compétition Inter-Box', content: 'Rappel : Qualifications ce samedi à 10h ! Préparez vos gilets lestés.', date: '2026-06-06', author: 'Head Coach' },
    { id: 2, title: 'Horaires Fériés', content: 'La box sera ouverte uniquement pour le WOD Hero de 10h.', date: '2026-06-04', author: 'Staff' }
  ]);
  ...
  return (
    <div className="space-y-4 pb-12">
      <div className="bg-gradient-to-r from-cyan-950/80 to-neutral-900 border border-cyan-500/30 rounded-3xl p-5 text-white shadow-xl">
        <div className="flex items-center gap-2 text-cyan-400 font-bold text-xs uppercase tracking-wider mb-1">
          <Zap className="w-4 h-4" /> Univers CrossFit & BoxWars
        </div>
        <h2 className="text-xl font-black">Espace Box & WODs</h2>
        <p className="text-xs text-neutral-300 mt-1">Valide tes scores WOD, suis tes records d'haltéro et consulte l'actu de la box.</p>
      </div>

      <div className="flex gap-2 border-b border-neutral-800 pb-2">
        <button onClick={() => setBoxSubTab('wods')} className={`px-3.5 py-2 rounded-xl text-xs font-bold transition ${boxSubTab === 'wods' ? 'bg-cyan-500 text-neutral-950' : 'bg-neutral-900 text-neutral-400 hover:text-white'}`}>WODs & Scores</button>
        <button onClick={() => setBoxSubTab('prs')} className={`px-3.5 py-2 rounded-xl text-xs font-bold transition ${boxSubTab === 'prs' ? 'bg-cyan-500 text-neutral-950' : 'bg-neutral-900 text-neutral-400 hover:text-white'}`}>Mes PRs 🎯</button>
        <button onClick={() => setBoxSubTab('feed')} className={`px-3.5 py-2 rounded-xl text-xs font-bold transition ${boxSubTab === 'feed' ? 'bg-cyan-500 text-neutral-950' : 'bg-neutral-900 text-neutral-400 hover:text-white'}`}>Annonces Box</button>
      </div>
      ...
      {boxSubTab === 'wods' && (
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setShowWodModal(true)} className="bg-cyan-500 hover:bg-cyan-400 text-neutral-950 px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-1 transition active:scale-95">
          ...
          {showWodModal && (
            <form onSubmit={handleAddBoxWod} className="bg-neutral-900 border border-neutral-800 p-4 rounded-2xl space-y-3 shadow-2xl animate-scaleUp">
              ...
              <div>
                <label className="block text-[10px] text-neutral-400 mb-1">Nom du WOD (ex: Fran, Murph, Cindy...) :</label>
          ...
          {loadingBoxWods ? (
            ...
            boxWods.map(wod => (
              <div key={wod.id} className="bg-neutral-900 border border-neutral-800 p-3.5 rounded-2xl flex justify-between items-center">
                <div>
      ...
      {boxSubTab === 'prs' && (
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setShowPrModal(true)} className="bg-cyan-500 hover:bg-cyan-400 text-neutral-950 px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-1 transition active:scale-95">
          ...
          {userPrs.length === 0 ? (
            ...
            userPrs.map(pr => (
              <div key={pr.id} className="bg-neutral-900 border border-neutral-800 p-3.5 rounded-2xl flex justify-between items-center">
      ...
      {boxSubTab === 'feed' && (
        <div className="space-y-3">
          ...
          {boxAnnouncements.map(ann => (
            <div key={ann.id} className="bg-neutral-900 border border-neutral-800 p-4 rounded-2xl space-y-2">
              <div className="flex justify-between items-center">
