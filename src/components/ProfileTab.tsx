...
import { Camera, ShieldCheck, ImageIcon, Key, LogOut, EyeOff, Share2, Camera as CamIcon, MapPin, Target } from 'lucide-react';
    ...
    <div className="space-y-4">
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6 text-center space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-orange-400 mt-1 flex items-center justify-center gap-1"><MapPin className="w-3.5 h-3.5" /> {currentUserProfile?.home_club || 'Club Tournai (Bastion)'}</p>
        </div>
      </div>
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-5 space-y-4">
        ...
        <form onSubmit={onAddTransformation} className="bg-neutral-950 p-4 rounded-2xl border border-neutral-800 space-y-3">
          <div className="grid grid-cols-2 gap-2.5">
            <button type="button" onClick={() => onCameraStart('trans_before')} className="p-3.5 bg-neutral-900 border border-neutral-800 rounded-xl text-xs text-neutral-300 flex items-center justify-center gap-2"><CamIcon className="w-4 h-4 text-orange-500" /> {newTransBefore ? '(✓)' : 'Avant'}</button>
            <button type="button" onClick={() => onCameraStart('trans_after')} className="p-3.5 bg-neutral-900 border border-neutral-800 rounded-xl text-xs text-neutral-300 flex items-center justify-center gap-2"><CamIcon className="w-4 h-4 text-orange-500" /> {newTransAfter ? '(✓)' : 'Après'}</button>
          </div>
