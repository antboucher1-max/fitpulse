...
export default function AnatomicIllustration({ muscleGroup, className = "w-full h-36" }: AnatomicIllustrationProps) {
  ...
  // Sélection du schéma SVG selon le groupe musculaire
  const getMuscleSvg = () => {
    if (normalize.includes('dos') || normalize.includes('back')) {
      return (
        <svg viewBox="0 0 100 100" className="w-full h-full text-orange-500 fill-current opacity-80">
          {/* Silhouette Dos / Lats */}
          ...
          <circle cx="50" cy="18" r="7" className="text-neutral-500 fill-current" />
        </svg>
      );
    } 
    ...
    if (normalize.includes('jambe') || normalize.includes('cuisse') || normalize.includes('quadriceps') || normalize.includes('leg')) {
      return (
        <svg viewBox="0 0 100 100" className="w-full h-full text-orange-500 fill-current opacity-80">
          ...
          <circle cx="50" cy="15" r="6" className="text-neutral-500 fill-current" />
        </svg>
      );
    } 
    
    if (normalize.includes('pectoral') || normalize.includes('chest') || normalize.includes('biceps') || normalize.includes('bras')) {
      return (
        <svg viewBox="0 0 100 100" className="w-full h-full text-orange-500 fill-current opacity-80">
    ...
    // Vue générique Athlète / Musculation
    return (
      <svg viewBox="0 0 100 100" className="w-full h-full text-orange-500 fill-current opacity-80">
        <path d="M50 12 C42 12 37 20 37 28 L32 50 L38 52 L42 35 L47 35 L47 85 L53 85 L53 35 L58 35 L62 52 L68 50 L63 28 C63 20 58 12 50 12 Z" opacity="0.3" />
        <circle cx="50" cy="22" r="10" className="text-orange-500 fill-current animate-pulse" />
        <circle cx="50" cy="15" r="6" className="text-neutral-500 fill-current" />
      </svg>
    );
  };
  ...
  return (
    <div className={`relative bg-neutral-950 border border-neutral-800 rounded-2xl flex flex-col items-center justify-center p-3 overflow-hidden ${className}`}>
      ...
      <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between text-[10px] text-neutral-400 font-bold z-10">
        ...
        <span className="text-[9px] text-orange-400 bg-orange-500/10 px-1.5 py-0.5 rounded border border-orange-500/20">Anatomie 3D</span>
      </div>
    </div>
  );
}
