...
export default function LiveTrackerTab({
  liveWorkoutName, setLiveWorkoutName, liveElapsedSeconds, liveExercises,
  selectedExToAdd, setSelectedExToAdd, exercisesDatabase, onAddExercise,
  onAddSet, onToggleSet, onUpdateWeight, onUpdateReps, onFinishWorkout, onQuitLive
}: LiveTrackerTabProps) {
  return (
    <div className="space-y-4">
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-5 space-y-4">
        ...
        <input type="text" value={liveWorkoutName} onChange={(e) => setLiveWorkoutName(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-2.5 text-sm font-bold text-white focus:border-orange-500" placeholder="Nom de la séance..." />
        ...
        <div className="bg-neutral-950 p-4 rounded-2xl border border-neutral-800 space-y-3">
          ...
          <div className="flex gap-2">
            <select value={selectedExToAdd} onChange={(e) => setSelectedExToAdd(e.target.value)} className="flex-1 bg-neutral-900 border border-neutral-800 rounded-xl px-3 py-2.5 text-xs text-white">
                ...
                <div className="space-y-2">
                  ...
                  {ex.sets.map((set, sIdx) => (
                    <div key={sIdx} className={`grid grid-cols-12 gap-2 items-center p-2 rounded-xl border ${set.completed ? 'bg-green-950/20 border-green-500/40' : 'bg-neutral-900 border-neutral-800'}`}>
                      ...
                      <div className="col-span-4"><input type="number" value={set.weight} onChange={(e) => onUpdateWeight(ex.id, sIdx, Number(e.target.value))} className="w-full bg-neutral-950 border border-neutral-800 rounded-lg py-1.5 text-center text-xs text-white" /></div>
                      <div className="col-span-4"><input type="number" value={set.reps} onChange={(e) => onUpdateReps(ex.id, sIdx, Number(e.target.value))} className="w-full bg-neutral-950 border border-neutral-800 rounded-lg py-1.5 text-center text-xs text-white" /></div>
                      <div className="col-span-2 flex justify-center"><button onClick={() => onToggleSet(ex.id, sIdx)} className={`p-2 rounded-xl transition ${set.completed ? 'bg-green-600 text-white' : 'bg-neutral-800 text-neutral-400'}`}><Check className="w-4 h-4" /></button></div>
                    </div>
                  ))}
                </div>
              </div>
